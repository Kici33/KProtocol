...
class S61SetTitleSubtitlePacket {
public:
    std::string text;

    [[nodiscard]] Packet to_packet(const ProtocolVersion wire_version) const {
#if defined(KPROTOCOL_HAS_GENERATED_CATALOG)
        const std::string_view key = generated::packet_keys::play_clientbound_set_title_subtitle;
#else
        const std::string_view key = "play.clientbound.set_title_subtitle";
#endif
        PacketFields fields;
    ...
    static S61SetTitleSubtitlePacket from_packet(const Packet& packet) {
        S61SetTitleSubtitlePacket out;
